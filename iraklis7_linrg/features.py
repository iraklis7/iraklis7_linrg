...
def main(
    ...
    data_tr["Όροφος Ρετιρέ"] = data_tr.apply(
        lambda row: is_top_floor(row["Όροφος"], row["Σύνολο ορόφων"]), axis=1
    )
    ...
    data_tr["Κατάσταση"] = data_tr.apply(
        lambda row: get_condition(row["Έτος κατασκευής"], row["Έτος ανακαίνισης"]), axis=1
    )
    data_tr[["Κατάσταση", "Έτος κατασκευής", "Έτος ανακαίνισης"]].head()

    # Return true if elevator exists and floor is third or higher
    data_tr["Ασανσέρ από 3ο"] = data_tr.apply(
        lambda row: elevator_from_third(row["Όροφος"], row["Ασανσέρ"]), axis=1
    )
    ...
    # Generate plot
    features_sel = features_norm[
        ["Εμβαδόν", "Θέα", "Όροφος Ρετιρέ", "Κατάσταση", "Ασανσέρ από 3ο"]
    ]
    plots.gen(
        list(features_sel),
